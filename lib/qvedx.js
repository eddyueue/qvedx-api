...
parseCURLError = function(err,stdout,stderr){
  ...
  if(match){
      ...
      if(statusCode == 0){
          ...
          return createErrorObject("CURLE_COULDNT_RESOLVE_HOST",msg,statusCode);
      } else if(statusCode == 28){
          return createErrorObject("CURLE_OPERATION_TIMEDOUT", msg, statusCode);
      } else {
          return createErrorObject("UNKNOWN", msg, statusCode);
      }
  } else {
      return createErrorObject("UNKNOWN", stderr);
  }
}

createErrorObject = function(msg,code,type){
  ...
  code = code || 0;
  type = type || '';
  var error = new Error(type + " Message: " + msg + " [Code: " + code + "]");

  //Clean-Up Stack Trace
  var stackTrace = error.stack.split("\n");
  error.stackTrace = [];
  stackTrace.forEach(function(element,index){
      if(element.indexOf('createErrorObject') == -1){
          error.stackTrace.push(element)
      }
  });
  error.stack = error.stackTrace.join("\n");
  return error;
}
...
createCurlRequest = function(options){
  ...
  if(QVEDX.trace){
    cmds.push('--trace-ascii ' + QVEDX.curlTraceFile);
  }
...
var serviceTypes = {
  ...
  DSC: 'QlikViewDirectoryServiceConnector',
 ...
 /**
   ...
   *  @param {bool} [options.curlTrace=false] - Enable CURL Trace Log
   *  @param {string} [options.curlTraceFile=trace.log] - CURL Trace File
   ...
   *  @param {string} [options.dateFormat=DD.MM.YYYY HH:mm:ss] - Date Format
   *  @param {string} [options.durationFormat=HH:mm:ss] - Format for Durations
   *  
   * @alias module:QVEDX
   */
function QVEDX(options){
  ...
  QVEDX.trace = options.curlTrace || false;
  QVEDX.verbose = options.verbose || false;
  ...
  QVEDX.dateFormat = options.dateFormat || "DD.MM.YYYY HH:mm:ss"
  QVEDX.durationFormat = options.durationFormat || "HH:mm:ss"

  QVEDX.curlTraceFile = options.curlTraceFile || "trace.log"
...
QVEDX.prototype.Ping = function(options,callback){
  ...
  QVEDX.prototype.GenericAPICall({
    soapAction: 'Ping',
    data: '<Ping xmlns=\\\"'+QVEDX.defaultNamepsace+'\\\"/>'
  },callback);
}

QVEDX.prototype.FindEDX = function(options,callback){
  if(!options.name){
    return callback(createErrorObject('options.name not defined'));
  }
 ...
 * @param {function} [callback] - Callback
 *  @param {object} callback.err - Error
 *  @param {object} callback.value - Trigger Result
 *  @param {object} callback.value.ExecId - Trigger Result ExecId
 */
QVEDX.prototype.TriggerEDXTask = function(options,callback){
  if(!options.taskNameOrID){
    return callback(createErrorObject('options.taskNameOrID not defined'));
  }
  ...
  //Intercept callback and manipulate response object
  var interceptCallback = function(err,triggerResult){
    if(!err){
      triggerResult.ExecID = triggerResult.ExecId;
      triggerResult.executionID = triggerResult.ExecId;
    }
    callback(err,triggerResult);  
  }
 ...
 *
 * @example //Trigger EDX Task
 *  var status = wait.for(q.GetEDXTaskStatus,{
 *      executionID: "MY ExecId"
 *  });
 *ö
 * @example //Trigger EDX Task
 *  q.GetEDXTaskStatus({
 ...
 * 
 * @param {object} options - Options
 *  @param {string} options.executionID - ExecId from Trigger Result
 * @param {function} [callback] - Callback
 *  @param {object} callback.err - Error
...
QVEDX.prototype.GetEDXTaskStatus = function(options,callback){
  if(!options.executionID){
    return callback(createErrorObject('options.executionID not defined'));
  }

  //Intercept callback and manipulate response object
  var interceptCallback = function(err,status){
    if(!err){
      //TODO calcualate duration properly
      var startMoment = moment(status.StartTime,QVEDX.dateFormat);
      var endMoment = moment(status.FinishTime,QVEDX.dateFormat);
      if(endMoment.isValid()){
        endMoment = moment();
      }
      
      var duration = moment(endMoment.diff(startMoment));
      status.duration = moment.duration(duration, "minutes").format();
...
QVEDX.prototype.GetTaskStatus = function(options,callback){
  if(!options.taskID){
    return callback(createErrorObject('options.taskID not defined'));
  }
...
QVEDX.prototype.GetCALConfiguration = function(options,callback){
  if(!options.qvsID){
    return callback(createErrorObject('options.qvsID not defined'));
  }
 ...
 *  @param {object} [options.scope=All] - Scope
 * @param {function} callback - Callback
 *  @param {object} callback.err - Error
...
QVEDX.prototype.GetDocumentMetaData = function(options,callback){
  if(!options.documentNode){
    return callback(createErrorObject('options.documentNode not defined'));
  }   
  if(!options.scope){
    options.scope = 'All'
  }
 ...
 * 
 * @param {object} options - Options
 *  @param {object} options.user - user
 *  @param {object} options.qvsID - QVS ID
 * @param {function} callback - Callback
 *  @param {object} callback.err - Error
 *  @param {object} callback.value - Document Meta Data
 */
QVEDX.prototype.GetCALInfoForUser = function(options,callback){
  if(!options.user){
    return callback(createErrorObject('options.user not defined'));
  }
  ...
  var data = '<GetCALInfoForUser xmlns=\\\"'+QVEDX.defaultNamepsace+'\\\"><qvsID>{qvsID}</qvsID><user>{user}</user></GetCALInfoForUser>';

  QVEDX.prototype.GenericAPICall({
    soapAction: 'GetCALInfoForUser',
    data: expandTemplate(data,options)
  },callback);    
}
 ...
 * @param {object} options - Options
 *  @param {object} options.user - user
 *  @param {object} options.dscID - DSC ID
 * @param {function} callback - Callback
 *  @param {object} callback.err - Error
 *  @param {object} callback.value - Document Meta Data
 */
QVEDX.prototype.LookupNames = function(options,callback){
  if(!options.user){
    return callback(createErrorObject('options.user not defined'));
  }

  if(!options.dscID){
    return callback(createErrorObject('options.dscID not defined'));
  }

  var data = '<LookupNames xmlns=\\\"'+QVEDX.defaultNamepsace+'\\\"><dscID>{dscID}</dscID><names xmlns:a=\\\"http://schemas.microsoft.com/2003/10/Serialization/Arrays\\\" xmlns:i=\\\"http://www.w3.org/2001/XMLSchema-instance\\\"><a:string>{user}</a:string></names></LookupNames>';

  QVEDX.prototype.GenericAPICall({
    soapAction: 'LookupNames',
    data: expandTemplate(data,options)
  },callback);    
}
...
QVEDX.prototype.GenericAPICall = function (options,callback){
  if(!options.soapAction){
    return callback(createErrorObject('options.soapAction not defined'));
  }
  ...
  if(!options.data){
    return callback(createErrorObject('options.data not defined'));
  }

  var callId = uuid.v1();
  console.log('---- QVEDX-API-Request: ' + callId + ' ' + options.soapAction);
  ...
  child = exec(cmd, function (err, stdout, stderr) {
      ...
      if(stdout == ""){
        ...
        //QMS API returns in some cases empty results. Very ugly
        return callback(createErrorObject("Empty response from QMS API received"));
      }
      ...
      //try {
        parseString(stdout, QVEDX.parserOptions,  function (err, result) {
          ...
          try {
            ...
            return callback(createErrorObject(ex));
          }
          return callback(err,soapResult);
        });
/*      } catch(ex) {
        callback(createErrorObject(ex));
      } */
  });
}
...
//LookupNames
//<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/"><s:Body><LookupNames xmlns="http://ws.qliktech.com/QMS/11/"><dscID>03a8f05a-4f00-4193-8df8-c2d07c2a7f82</dscID><names xmlns:a="http://schemas.microsoft.com/2003/10/Serialization/Arrays" xmlns:i="http://www.w3.org/2001/XMLSchema-instance"><a:string>ww002\albisser</a:string><a:string>roman.albisser@siemens.com</a:string><a:string>z000dk3q</a:string><a:string>albisser*</a:string></names></LookupNames></s:Body></s:Envelope>
