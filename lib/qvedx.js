...
QVEDX.prototype.GetDocumentMetaData = function(options,callback){
	...
	var data = '<GetDocumentMetaData xmlns=\\\"'+QVEDX.defaultNamepsace+'\\\"><userDocument xmlns:a=\\\"http://schemas.datacontract.org/2004/07/PIX.QMSAPI.DataObjects\\\" xmlns:i=\\\"http://www.w3.org/2001/XMLSchema-instance\\\"><a:FolderID>{FolderID}</a:FolderID><a:ID>{ID}</a:ID><a:IsOrphan>{IsOrphan}</a:IsOrphan><a:IsSubFolder>{IsSubFolder}</a:IsSubFolder><a:Name>{Name}</a:Name><a:RelativePath>{RelativePath}</a:RelativePath><a:TaskCount>{TaskCount}</a:TaskCount><a:Type>{Type}</a:Type></userDocument><scope>Licensing</scope></GetDocumentMetaData>';
